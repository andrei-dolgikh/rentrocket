...
export function AdminViewsStatBox({ chartName, header }: ChartsBoxProps) {
  ...
  const chartData = [["Datetime", "Count"], ...statistics.dashStat.map((stat: any) => [new Date(stat.timestamp).getTime(), stat.views])];
  
  const lastViewsCount = statistics.dashStat[statistics.dashStat.length - 1]?.views;
  return (
        ...
        <>
          <div className="flex flex-row justify-between p-4">
            <div className="">
              ...
              <p className="text-[24px] text-semibold">{lastViewsCount}</p>
            </div>
